...
export class InviteComponent implements OnInit, AfterViewChecked{
    ...
    createLink(){
        ...
        if(typeof this.ocode !== "undefined"){
            this.link = `${this.appService.environment.url}/event/${this.eventId};ocode=${this.ocode}`;
        }
        else{
            this.link = `${this.appService.environment.url}/event/${this.eventId}/${this.eventId}`;
        }
