...
import {TicketRegistration} from '../../event/models/ticket-registration';
...
import {Promoter} from '../../event/models/promoter';
...
export class EventService {
    ...
    emailTemplateDrafts$ : FirebaseListObservable<any>;
    emailTemplateDrafts:any[]=[];
    ...
    addEmailTemplateDraft(draft){
      this.emailTemplateDrafts$.push(draft);
    }

    getEmailTemplateDrafts(): FirebaseListObservable<any>{
      return this.af.list(`/events/communication/${this.eventId}`);
    }
    ...
    setEvent(eventId: string, draft: boolean = false): Promise<any>{
        ...
        this.emailTemplateDrafts$ = this.getEmailTemplateDrafts();
        this.emailTemplateDrafts$.subscribe((data)=> {
            this.emailTemplateDrafts =data;
        });
    ...
    addTicketRegistration(ticket: any, ticketType, index, multiRegistrant, buyMultiple) {
        for (let i = 0; i < parseInt(this.currentTickets[index].quantity, 10); i++) {
