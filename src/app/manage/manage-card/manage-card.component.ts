                ...
                <div class="col-md-8">
                    <div class="c-info-list">
                        ...
                        <div *ngIf="eventType=='live' || eventType=='past'">
                            ...
                            <div style="font-size:12px;">{{ ticketUsed }}/{{ event.totalTickets }} registrations filled</div>
                        </div>
                    ...
                    <div class="low" *ngIf="eventType=='live'" style="margin-top:5px;"> 
                        <a routerLink="/event/my-event/{{eventId}}"><i class="fa fa-gear"></i> Manage</a> &nbsp; &nbsp; 
                        <a routerLink="/event/my-event/{{eventId}}/invitations"><i class="fa fa-users"></i> Invite</a>                        
                    </div>
    ...
    setEvent(){
        if(this.eventType === "live" || this.eventType === "promotedLive" || this.eventType == "attendingLive" || this.eventType == "bookmark"){
            this.af.object(`/events/live/${this.eventId}`).subscribe((data)=>{
                if(data.$exists()){
                    console.log(data);
                    this.ticketUsed = data.ticketsUsed;
        ...
        } else if(this.eventType === "attendingHistory"){
            this.af.object('/events/history/' + this.eventId).subscribe(
                data => {
                    if(data.$exists()){
                        this.event = data;
                        this.set = true;
