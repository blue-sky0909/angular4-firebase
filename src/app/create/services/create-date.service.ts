...
export class CreateEventDateService {
    ...
    groomDate(data, type){
        ...
        const buildMonthlyDayString = function(data){
            ...
            const dateString = weeksString + daysString + ` ${moment(data.cycleStartDate).format('MM/DD/YY')} - ${moment(data.cycleEndDate).format('MM/DD/YY')}`;
            const timeString = `${moment(`2012-12-12 ${data.dailyStartTime}`).format('h:mm A')} - ${moment(`2012-12-12 ${data.dailyEndTime}`).format('h:mm A')}`;
        ...
        const single = function(){
            ...
            const timeString = `${moment(`2012-12-12 ${startTime}`).format('h:mm A')} - ${moment(`2012-12-12 ${endTime}`).format('h:mm A')}`;
        ...
        const weekly = function(){
            ...
            const timeString = `Every ${days} from ${moment(`2012-12-12 ${data.start.time}`).format('h:mmA')} to ${moment(`2012-12-12 ${data.end.time}`).format('h:mmA')}.`;
        ...
        const biweekly = function(){
            ...
            const dateString = `${moment(data.start.date).format('MM/DD/YY')} - ${moment(data.end.date).format('MM/DD/YY')}`;
            const timeString = `Every other ${days} from ${moment(`2012-12-12 ${data.start.time}`).format('h:mmA')} to ${moment(`2012-12-12 ${data.end.time}`).format('h:mmA')}.`;
        ...
        const monthlyDate = function(){
            const dateString = `Occurs the ${data.repeatOn} of each month.  Starts ${moment(data.cycleStartDate).format('MMM Do, YYYY')} and ends ${moment(data.cycleEndDate).format('MMM Do, YYYY')}`;
            const timeString = `${moment(`2012-12-12 ${data.dailyStartTime}`).format('h:mmA')} to ${moment(`2012-12-12 ${data.dailyEndTime}`).format('h:mmA')}`;
        ...
        const custom = function () {
            ...
            const dateString = `Custom Dates from ${moment.unix(startDate).format('MM/DD/YY')} to ${moment.unix(endDate).format('MM/DD/YY')}`;
    ...
    getDates(data: IDraftDate): any{
        ...
        const getMonthDate = function(data){
            ...
            if (repeatOn >= beginningDay){
                const firstEventDate = `${moment(monthlyData.cycleStartDate).format('YYYY')}-${moment(monthlyData.cycleStartDate).format('MM')}-${repeatDD}`;
