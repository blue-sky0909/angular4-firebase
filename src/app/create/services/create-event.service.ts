...
export class CreateEventService{
    ...
    groomedDate: any;
    eventType: any;
    haveTickets : ReplaySubject<any> = new ReplaySubject(1);
    locationType : ReplaySubject<any> = new ReplaySubject(1);
    saveChanges : ReplaySubject<any> = new ReplaySubject(1);
    liveEventValue:boolean = false;
    ...
    setDateTime(groomedDate: GroomedDate, eventType: string, endDate){
      this.groomedDate = groomedDate;
    ...
    validateDate(){
        ...
        if (this.eventType === 'singleDay'){
            const currentDate = new Date();
            if (moment(currentDate).unix() > this.groomedDate.startDate){
        ...
        }else if (this.eventType === 'multiDay'){
            const currentDate = new Date();
            if (moment(currentDate).unix() > this.groomedDate.startDate){
            ...
            }else if (this.groomedDate.endDate <= this.groomedDate.startDate){
                this.fieldValidations.get('event-date').valid = false;
    ...
    validateTime(): boolean{
        console.log(this.groomedDate);
        const times = this.groomedDate.timeString.split('-');
        const minsDifference = (moment(`2012-12-12 ${times[1]}`).toDate().getTime() - moment(`2012-12-12 ${times[0]}`).toDate().getTime()) / 60000;
    ...
    validateLive(): boolean{
        ...
        console.log(this.liveEventValue);
        if(!this.liveEventValue){
            this.ticketService.ticketList$.subscribe(data => {
                if(data.length > 0){
                    let quantity = true;
                    for(let i = 0; i < data.length; i++){
                        if(data[i].ticketQuantity <= 0){
                            quantity = false;
                        }
                    }
                    if(quantity){
                        this.updateHaveTickets(true);
                    } else{
                        this.valid = false;
                        this.updateHaveTickets(false);
        ...
        if(this.draft.category.trim() == ''){
            this.setErrorInField('category', 'Category is required');
            this.valid = false;
        } else{
            this.removeErrorInField('category');
        }
                    ...
                    this.af.object(`/events/components/date/${this.auth.id}/${data.dateKey}`).first().subscribe((dateData) => {
                        ...
                        if (!preventTwice){
                            ...
                            if (dateData.eventType !== 'recurring'){
                                ...
                                liveObject$.update({ref: this.eventId, ocode: this.appService.ocode, uid: this.auth.id, data: data, endDate: this.eventEndDate}).then((success) => {
    ...
    removeErrorInField(field:string){
        const fieldValidation: FieldValidation = new FieldValidation();
    ...
    updateHaveTickets(value){
        console.log(value);
        this.haveTickets.next(value);
    }

    updateLocationType(value){
        this.locationType.next(value);
    }

    updateSaveChanges(value){
        console.log(value);
        this.saveChanges.next(value);
    }
}
