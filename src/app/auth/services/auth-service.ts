...
export class AuthService {
  ...
  getDefaultImage(uniquePath): firebase.Promise<any>{
    return this.app.storage().ref(`defaultImages/${this.id}/${uniquePath}`).getDownloadURL();
  }
  ...
  uploadDefaultImage(data, uniquePath): Observable<any>{
    const subject = new Subject();
    const blob = this.base64toBlob(data.image.replace(/^data:image\/(png|jpg|jpeg);base64,/, ""), this.imageContentType);
    // const storageRef = this.app.storage().ref(`/userImages/${this.id}/${uniquePath}`);
    const storageRef = this.app.storage().ref(`/defaultImages/${uniquePath}`);
    storageRef.put(blob).then((fileData) => {
      subject.next(fileData);
    });
    return subject;
